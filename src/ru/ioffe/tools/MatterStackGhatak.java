...
//		intervals.add(0, new MatterInterval(-intervals.get(0).getWidth(), 0, 0, intervals.get(0).getMass(), intervals.get(0).getDiel()));
	 ...
	 * 
	 * @return MatterInterval instance of the fictitious interval that is always
	 *         0th in the MatterStack
	 */
	...
	/**
	 * Add an interval to the chosen position of the stack. Index should start
	 * from 1 in order not to affect fictitious 0th interval
	 * 
	 * @param index
	 *            position
	 * @param element
	...
	/**
	 * Add a new MatterInterval region, specifying its essential properties for
	 * constructor
	 * 
	 * @param width
	 *            width of the interval
	 * @param y
	 *            potential value in electron-volts
	 * @param mass
	 *            mass of a particle in units of free electron mass
	 * @param diel
	 *            dielectric constant of the interval
	 */
	...
	private void process() {
		...
		double y_min = intervals.get(1).getY(); // addressing to the first
												// non-fictitious interval
		MatterInterval inl;
	...
	public Complex getDelta(int index, double energy) {
		...
		if (index == 0) {
			...
			// FIXME Check sum of the intervals widths from 0 to index.
			// The intervals must not have equal widths, for now the opposite is
	...
	public double getPropagationEnergy(int index, double energy_min, double energy_max) {
		...
		while (Math.abs(dprop) > Function.CALC_TOLERANCE) {
			System.out.println("BSF: " + dprop + "\tenergy_min: " + energy_min + "\tenergy: " + energy + "\tenergy_max: "
					+ energy_max);
			if (Math.abs(energy / (energy_min + energy_max) - 0.5) < Function.CALC_TOLERANCE * 0.001) {
				return energy;
			}
	...
	public double[] getEigenEnergy() {
		...
		// Among all intervals we are looking for those which are truly quantum
		// wells, they bring all the information.
		for (int i = 1; i < intervals.size() - 1; i++) {
			if (intervals.get(i).getY() <= intervals.get(i - 1).getY() && intervals.get(i).getY() < intervals.get(i + 1).getY()) {
	...
	public double[][] getWaveFunction(double energy) {
		...
		Complex[] coef;
		Complex jp = new Complex(0, 1);
		Complex jm = new Complex(0, -1);
	...
	public static void main(String[] args) {
		...
		a.appendNewMatterInterval(2e-5, 1.4, 0.1002, diel);
